...
$BIN tx $BIN_TX open-contract -y -b block --from "$USER" --keyring-backend test -- "$PUBKEY" "$SERVICE" "$CLIENT_PUBKEY" "$CTYPE" "$DEPOSIT" "$DURATION" "$RATE""uuram" $QUERY_PER_MINUTE $SETTLEMENT_DURATION
